...
class Goals(torch.utils.data.Dataset):
    """
    Goals video loader. Construct the Goals video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=100):
        """
        Construct the Goals video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Goals".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing Goals {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load Goals split {} from {}".format(
